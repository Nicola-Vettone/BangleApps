...
// ============================================================
// TIMEZONE: Europe/Rome (CET/CEST automatico)
// ============================================================
function lastSundayOfMonthUTC(year, monthIndex0) {
  // monthIndex0: 0=Jan ... 11=Dec
  let d = new Date(Date.UTC(year, monthIndex0 + 1, 0, 1, 0, 0)); // last day, 01:00 UTC
  let dow = d.getUTCDay(); // 0 = Sunday
  d.setUTCDate(d.getUTCDate() - dow);
  return d;
}

function isEuropeDST(utcNow) {
  const y = utcNow.getUTCFullYear();
  const start = lastSundayOfMonthUTC(y, 2); // March
  const end   = lastSundayOfMonthUTC(y, 9); // October
  return utcNow >= start && utcNow < end;
}

function setRomeTimeZone() {
  // Italy: CET=+1, CEST=+2
  const utcNow = new Date(Date.now());
  E.setTimeZone(isEuropeDST(utcNow) ? 2 : 1);
}

// ============================================================
// SLEEP/WORN STATUS + ICON
// ============================================================
// sleepStatus: 0 unknown(?), 1 not worn(X), 2 awake, 3 light, 4 deep
let sleepStatus = 0;
let sleepLastEval = 0;
let bootMs = Date.now();
...
// più permissivo (evita X fissa)
const WORN_SCORE_MIN_WORN = 1;
const HRM_WORN_CONFIDENCE_MIN = 1;

// movement thresholds (tuning facile)
...
function drawSleepIcon() {
  ...
  // FIX: forza colori ogni volta (evita icona “sparita”)
  g.setColor(1,1,1);
  g.setBgColor(0,0,0);
...
function updateWornScoreFromHRM() {
  ...
  const bpm = hrm_1.bpm|0;
  const conf = hrm_1.confidence|0;

  // worn se conf>0 o bpm>0 o abbiamo già avuto un bpm valido
  const ok = (conf >= HRM_WORN_CONFIDENCE_MIN) || (bpm > 0) || (lastValidBPM > 0);
...
function evaluateSleepStatus() {
  const now = Date.now();
  // come sleeplog: finestra 10 minuti
  if (now - sleepLastEval < 10*60*1000) return;
  sleepLastEval = now;

  // primi 2 minuti: mostra ? se non è worn (evita X subito)
  if ((now - bootMs) < 2*60*1000 && !isWorn()) {
    sleepStatus = 0;
    return;
  }
...
function logBpmThrottled(v) {
  const now = Date.now();
  if (now - _lastBpmPrint < 2000) return;
  _lastBpmPrint = now;
...
// ============================================================
// CLOCK DRAW (orario + data + MAC + icona)
// ============================================================
function drawClock() {
  ...
  // FIX timezone Italia
  // (non costa nulla farlo spesso; comunque lo “rinforziamo” anche ogni ora nel loop)
  setRomeTimeZone();

  const date = new Date();
  const timeStr = require("locale").time(date, 1); // mantiene ":" e locale

  // FIX: usa il TUO font custom
  g.setFontAlign(0,0);
  g.setFontAnton(1);
...
function updateBLE() {
  NRF.updateServices({
    0x180D: {
      0x2A37: {
        value: [6, (hrm_1 && hrm_1.confidence >= 50)
...
function setupBLE() {
  NRF.setServices({
    ...
    "E95D0753251D470AA062FA1922DFA9A8": {
      "E95D0753251D470AA062FA1922DFA9A8": {
        value:[0,0,0,0,0,0], notify:true, readable:true
      }
    }
  }, { uart:false });
}
...
// FIX: accendi HRM subito (non aspettare il primo ciclo)
...
// Loop: come originale (10s)
setInterval(() => {
  // rinforza timezone 1 volta/ora (utile per cambio DST)
  if ((Date.now() % (60*60*1000)) < 10000) setRomeTimeZone();
