...
Graphics.prototype.setFontAnton = function(scale) { // Actual height 69 (68 - 0) 
...
// più permissivo (così non resti bloccato su X)
...
function updateWornScoreFromHRM() {
  ...
  // consideriamo "worn" anche con bpm=0 se conf>0,
  // oppure se abbiamo già avuto un BPM valido in passato
...
function evaluateSleepStatus() {
  const now = Date.now();
  if (now - sleepLastEval < 10 * 60 * 1000) return; // 10 minuti
  sleepLastEval = now;

  // per i primi 2 minuti, se non è worn mostra "unknown"
...
// ============================================================
// Original drawClock + icon at end
// ============================================================
function drawClock() {
  ...
  const date = new Date();
  const timeStr = require("locale").time(date, 1); // mantiene ":" e ora locale
...
// IMPORTANT: accendi HRM subito, non aspettare il primo ciclo
Bangle.setHRMPower(true, "always");

drawClock();
setTimeout(Bangle.drawWidgets, 0);

// Loop: aggiorna orologio + ciclo sensori ogni 10s (come originale)
setInterval(() => {
  drawClock();
  startSensorCycle();
  updateWornScoreFromHRM();
